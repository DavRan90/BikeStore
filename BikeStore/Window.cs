    ...
    public class Window
    {
        ...
        public void DrawBorder(int rows)
        {
            ...
            for (int j = 0; j < rows; j++)
            {
                ...
                Console.WriteLine('│' + " " + TextRows[0] + new String(' ', width - TextRows[0].Length + 1) + '│');
            }

            // Rita undre delen av fönstret
            Console.SetCursorPosition(Left, Top + rows + 1);
            Console.Write('└' + new String('─', width + 2) + '┘');


            // Kolla vilket som är den nedersta posotion, i alla fönster, som ritats ut
            if (Lowest.LowestPosition < Top + rows + 2)
            {
                Lowest.LowestPosition = Top + rows + 2;
            }
        ...
        public void Draw()
        {
            if (TextRows.Count > 0)
            {
                ...
                if (Header != "")
                {
                    ...
                    Console.WriteLine(new String(' ', width - Header.Length + 1) + '│');
                    Console.SetCursorPosition(Left, Top);
                    Console.Write('└' + new String('─', width + 2) + '┘');
                    Console.ForegroundColor = ConsoleColor.White;
                }
